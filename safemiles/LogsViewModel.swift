...
class LogsViewModel: ObservableObject {
    ...
    init() {
        ...
        NotificationCenter.default.addObserver(forName: .logsUpdate, object: nil, queue: .main) {
            [weak self] _ in
    ...
    func fetchLogs(refresh: Bool = false) {
        ...
        APIManager.shared.request(url: ApiList.getLogs, method: .get, parameters: params) { comp in
            ...
            DispatchQueue.main.async {
                ...
                // Default to the latest available log date from API if no date is selected 
                // or if the current selectedDate is not represented in availableDates.
                // We check day, month, year to avoid sub-second comparison issues with Date().
                let isInitialOrToday = Calendar.current.isDateInToday(self.selectedDate) || self.availableDates.isEmpty
                
                if let lastDate = self.availableDates.last {
                    if isInitialOrToday || !self.availableDates.contains(where: { Calendar.current.isDate($0, inSameDayAs: self.selectedDate) }) {
    ...
    private func calculateDutySegments() {
        ...
        let events = allEvents.filter {
            $0.code?.lowercased() == "d" || $0.code?.lowercased() == "on"
                || $0.code?.lowercased() == "sb" || $0.code?.lowercased() == "off"
                || $0.code?.lowercased() == "ym" || $0.code?.lowercased() == "pu"
        }
        ...
        for (index, value) in events.enumerated() {
            ...
            dutySegments.append(
                DutySegment(
                    status: status, startHour: startHour, endHour: endHour, isDotted: isDotted))
        }
    }
    ...
    private func addInitialEvents(code: String, initialTime: String, noEvents: Bool = false) {
        ...
        dutySegments.append(
            DutySegment(status: status, startHour: startHour, endHour: endHour, isDotted: isDotted))
    }
    ...
    func saveForm(
        ...
        APIManager.shared.upload(
            url: ApiList.saveForms + logId + "/", method: .patch, parameters: params
        ) { comp in
            // completion
        } success: { response in
            self.isLoading = false
    ...
    func certifyLog(signature: UIImage, completion: @escaping (Bool, String) -> Void) {
        ...
        APIManager.shared.upload(
            ...
            self.isLoading = false
            let success =
                (response["success"] as? Bool == true) || (response["certified"] as? Bool == true)
            if success {
