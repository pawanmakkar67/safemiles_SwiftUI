                ...
                if let violations = viewModel.currentLog?.violations, !violations.isEmpty {
                    Button(action: {
                        ...
                        HStack(spacing: 12) {
                            ...
                            // Text
                            Text(
                                "Violation Alert : \(violations.count) violation\(violations.count == 1 ? "" : "s") recorded"
                            )
...
struct EventRow: View {
    ...
    func calculateDuration() -> String? {
        ...
        guard
            ...
            let endDate = formatter.date(from: endStr) ?? ISO8601DateFormatter().date(from: endStr)
        else { return nil }
